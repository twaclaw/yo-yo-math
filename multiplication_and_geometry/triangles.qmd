...
But proving that one is a bit more difficult, so we are going to leave it for later! $$\text{[wink:2em:0em:2]{.twemoji}}$$
...
for _ in range(4):
    display_vector(draw_right_triangle(W, H, all_labels_inside=True, dimension_labels=("a", "b", "c"), show_hypotenuse_dimension=True, cell_size=CELL_SIZE))
```
...
# Basic rectangle with dimensions and grid
display_vector(draw_rectangle(W+H, H+W, dimension_labels=("", ""), show_grid=False, cell_size=CELL_SIZE))
```
