...
Let's do some bricolage! Print this page, then cut out the colored squares. Then, glue them together so that the colors match. It's helpful to print two sets so you can make comparisons. Leave the white square on the page.
...
For example, flipping the square along the axis $A$ results in:
...
from yoyo_plots.permutations import R4, SquareSymmetries, FlipAxis
from yoyo_plots.common import display_vector, combine_svgs
...
The symmetries of the square, $S_G = \{R_0, R_1, \cdots R_7\}$, forms a group under the operation of function composition, [$\color{red}{\circ}$]{.scale factor="2"}. Remember our checklist? Let's take a look at the Cayley table of the group.
...
:::{.fun-fact title="Dihedral group"}
This group of symmetries is called the _dihedral_  group of order $8$ and is denoted $D_4$. For any polygon of $n$ sides, the dihedral group with $2n$ elements is denoted $D_n$ or $D_{2n}$.
:::

:::{.question title="Why is called diehedral?"}
I'll leave this one as an exercise for you to do some research on.
:::
